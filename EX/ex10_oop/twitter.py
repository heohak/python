...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    result = 0
    for tweet in tweets:
        sum1 = tweet.retweets / tweet.time
        if sum1 > result:
            result = sum1
            final = tweet
    return final
...
def sort_by_popularity(tweets: list) -> list:
    ...
    """
    return sorted(tweets, key=lambda t: t.retweets, reverse=True)
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    dict1 = {}
    for tweet in tweets:
        list1 = tweet.content.split()
        for word in list1:
            if word.startswith("#"):
                if word not in dict1:
                    dict1[word] = [tweet.retweets]
                elif word in dict1:
                    dict1[word].append(tweet.retweets)
