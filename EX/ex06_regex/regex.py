...
def find_words_from_sentence(sentence: str) -> list:
    ...
    """
    pattern = r"\w+"
    result_list = re.findall(pattern, sentence)
    return result_list
...
def find_words_from_sentences_only(text: str) -> list:
    ...
    sentences_as_list = str(sentences)
    words = find_words_from_sentence(sentences_as_list)
    return words
...
def find_years(text: str) -> list:
    ...
    """
    pattern = r"(?<!\d)\d{4}(?!\d)"
    strings_list = re.findall(pattern, text)
    ints_list = list(map(int, strings_list))
    return ints_list
